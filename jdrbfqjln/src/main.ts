...
import { InventorySystem, DebugLogics, playerBag, getWeaponBoost, getArmorDefBoost, getArmorPpBoost, type EquipmentItem, type ManuscriptItem, getGradeMultiplier } from './inventory';
...
export class Entity {
    ...
    get totalAtk(): number { 
        ...
        const armorBonus = armorItem && armorsDB[armorItem.id].atkBonus ? armorsDB[armorItem.id].atkBonus! * getGradeMultiplier(armorItem.grade) : 0;
    ...
    get totalDef(): number { 
        ...
        if (this.activeModifiers.includes('tenderized')) total = Math.max(1, Math.floor(total * 0.85));
        return Math.max(1, total); 
    }
    // --- NOUVEAUTÉ : Le calcul des PP Max restauré ---
    ...
    drawStatsBars(ctx: CanvasRenderingContext2D) { const barWidth = this.width; const barX = this.x; let barY = this.y - 20; ctx.fillStyle = '#333'; ctx.fillRect(barX, barY, barWidth, 6); ctx.fillStyle = '#2ecc71'; ctx.fillRect(barX, barY, barWidth * Math.max(0, this.hp / this.maxHp), 6); ctx.strokeStyle = '#000'; ctx.lineWidth = 1; ctx.strokeRect(barX, barY, barWidth, 6); ctx.fillStyle = 'white'; ctx.font = 'bold 12px Arial'; ctx.fillText(`Lv.${this.level}`, barX - 35, barY + 7); if (this.totalMaxPp > 0) { barY += 8; ctx.fillStyle = '#333'; ctx.fillRect(barX, barY, barWidth, 4); ctx.fillStyle = '#9b59b6'; ctx.fillRect(barX, barY, barWidth * Math.max(0, this.pp / this.totalMaxPp), 4); ctx.strokeRect(barX, barY, barWidth, 4); } }
