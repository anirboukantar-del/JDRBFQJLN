...
export const playerBag = {
    weapons: [] as EquipmentItem[],
    armors: [] as EquipmentItem[],
    items: [] as string[],
    manuscripts: [] as ManuscriptItem[]
};
...
export function getGroupedItems(): { id: string, count: number }[] {
    ...
    const order: string[] = []; 
    for (const id of playerBag.items) {
...
export function getGradeMultiplier(grade: Grade): number {
    ...
    if(grade === 'D') return 0.8; if(grade === 'E') return 0.6; if(grade === 'F') return 0.5; 
    return 1.0;
}
...
export const InventorySystem = {
    handleInput(key: string, player: any) {
        ...
        if (key === 'ArrowRight' || key.toLowerCase() === 'd') {
            if (currentTab === 'weapons') currentTab = 'armors'; 
            else if (currentTab === 'armors') currentTab = 'items';
            else if (currentTab === 'items') currentTab = 'manuscripts';
            selectedIndex = 0;
        }
        else if (key === 'ArrowLeft' || key.toLowerCase() === 'q') {
            if (currentTab === 'manuscripts') currentTab = 'items';
            else if (currentTab === 'items') currentTab = 'armors'; 
            else if (currentTab === 'armors') currentTab = 'weapons';
            selectedIndex = 0;
        }
            ...
            if (currentTab === 'weapons') { player.equippedWeapon = playerBag.weapons[selectedIndex]; player.hp = Math.min(player.hp, player.maxHp); player.pp = Math.min(player.pp, player.totalMaxPp); }
                ...
                const grouped = getGroupedItems(); const itemId = grouped[selectedIndex].id;
                const item = itemsDB[itemId]; let itemUsed = false;
...
// --- LOGIQUE DU MENU DEBUG ---
export const DebugLogics = {
    giveAllEquipment() {
        const wIds = Object.keys(weaponsDB);
        wIds.forEach(id => playerBag.weapons.push({ id, grade: 'A', level: 100 })); // Niveau 100
        
        const aIds = Object.keys(armorsDB);
        aIds.forEach(id => playerBag.armors.push({ id, grade: 'A', level: 100 })); // Niveau 100
    ...
    giveAllManuscripts() {
        ...
        sIds.forEach(id => playerBag.manuscripts.push({ skillId: id, level: 100 })); // Niveau 100
