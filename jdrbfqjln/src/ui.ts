...
import { playerBag, currentTab, selectedIndex, getGroupedItems, getWeaponBoost, getArmorDefBoost, calculatePpMult, inventorySubState, replaceTargetIndex, type EquipmentItem } from './inventory'; 
...
export const debugOptions = [ "Niveau Joueur +1", "Niveau Joueur -1", "Étage +1", "Étage -1", "Changer Type Map", "REGEN MAP (Appliquer)", "TOUT EQUIPEMENT (Lv.100)", "TOUS LES OBJETS (x5)", "TOUTES LES SPES (Lv.100)" ];
...
export const UIManager = {
    drawExplorationHUD(ctx: CanvasRenderingContext2D, canvasWidth: number, player: any, alertMessage: string, alertTimer: number, currentFloor: number, mandatoryRemaining: number, totalEnemies: number) {
        ...
        if (currentFloor > 0) {
            const triSize = 8; const gap = 10; const totalTriWidth = mandatoryRemaining * (triSize * 2) + Math.max(0, mandatoryRemaining - 1) * gap;
            let startX = canvasWidth / 2 - totalTriWidth / 2 + triSize;
            ctx.fillStyle = '#f1c40f';
            for(let i = 0; i < mandatoryRemaining; i++) { ctx.beginPath(); ctx.moveTo(startX - triSize, 55 - triSize); ctx.lineTo(startX + triSize, 55 - triSize); ctx.lineTo(startX, 55 + triSize); ctx.fill(); startX += (triSize * 2) + gap; }
            if (mandatoryRemaining === 0) { ctx.font = '16px Arial'; ctx.fillStyle = '#2ecc71'; ctx.fillText(`Escalier déverrouillé !`, canvasWidth / 2, 55); }
            ctx.font = '14px Arial'; ctx.fillStyle = '#bdc3c7'; ctx.fillText(`${totalEnemies} ennemis restants`, canvasWidth / 2, 75);
        } else { ctx.font = 'italic 18px Arial'; ctx.fillStyle = '#bdc3c7'; ctx.fillText(`Zone Sûre`, canvasWidth / 2, 68); }
        ctx.textAlign = 'left';

        if (alertTimer > 0) { ctx.fillStyle = 'rgba(46, 204, 113, 0.9)'; ctx.fillRect(canvasWidth / 2 - 150, 95, 300, 50); ctx.fillStyle = 'white'; ctx.font = 'bold 20px Arial'; ctx.textAlign = 'center'; ctx.fillText(alertMessage, canvasWidth / 2, 128); ctx.textAlign = 'left'; }
    },
    ...
    drawInteraction(ctx: CanvasRenderingContext2D, player: any, chest: any) {
        if (!chest.isOpen) {
            ...
            if (dist < 80) { ctx.fillStyle = 'rgba(0, 0, 0, 0.7)'; ctx.fillRect(player.x - 20, player.y - 45, 80, 25); ctx.fillStyle = 'white'; ctx.font = 'bold 12px Arial'; ctx.fillText("[X] Ouvrir", player.x - 10, player.y - 28); }
        }
    },
        ...
        if (listLength > 0) {
            ...
            if (currentTab === 'items') {
                ...
                if (currentTab === 'weapons') { 
                    ...
                    if (a.atkPenalty) extraInfo += ` | -${a.atkPenalty} ATK`; if (a.atkBonus) extraInfo += ` | +${a.atkBonus} ATK`;
                    if (a.spCostPenalty) extraInfo += ` | Spé Cost ${a.spCostPenalty > 0 ? '+' : ''}${Math.round(a.spCostPenalty * 100)}%`;
                    if (a.fearChanceBonus) extraInfo += ` | +Terreur`;
                }
            }
    ...
    drawCombatIntro(ctx: CanvasRenderingContext2D, canvasWidth: number, canvasHeight: number) {
        ...
        const progress = introAnimationTimer / INTRO_ANIMATION_DURATION; const radius = Math.sin(progress * Math.PI) * (canvasWidth / 2);
        ctx.save(); ctx.strokeStyle = `rgba(255, 255, 255, ${1 - progress})`; ctx.lineWidth = 10 * (1 - progress); ctx.beginPath(); ctx.arc(canvasWidth / 2, canvasHeight / 2, radius, 0, Math.PI * 2); ctx.stroke(); ctx.restore();
    },
    ...
    drawCombatMenu(ctx: CanvasRenderingContext2D, canvasWidth: number, canvasHeight: number, activePlayer: any, currentTargetIndex: number) {
        ...
        ctx.fillStyle = 'rgba(0, 0, 0, 0.85)'; ctx.fillRect(menuX, menuY, menuWidth, menuHeight); ctx.strokeStyle = '#fff'; ctx.lineWidth = 3; ctx.strokeRect(menuX, menuY, menuWidth, menuHeight);
        ctx.font = '22px Arial';
        ...
        if (combatSubState === 'ACTION_SELECT') {
            ...
            if (skills.length === 0) { ctx.fillStyle = 'gray'; ctx.fillText(`Aucune Spé...`, menuX + 30, menuY + 50); } 
            else {
                skills.forEach((sObj: any, index: number) => {
                    const dSkill = getDynamicSkill(sObj.id, sObj.level); const textY = menuY + 40 + (index * 40);
            ...
            ctx.fillStyle = '#f1c40f'; ctx.font = '16px Arial'; ctx.fillText(`[Haut/Bas] Changer cible`, menuX + 20, menuY + 110); ctx.fillText(`[Entrée] Valider   [Échap] Retour`, menuX + 20, menuY + 130);
        }
    },
    ...
    drawDebugMenu(ctx: CanvasRenderingContext2D, canvasWidth: number, canvasHeight: number, player: any, currentFloor: number, nextType: string, selectedIndex: number) {
        ctx.fillStyle = 'rgba(0, 0, 0, 0.85)'; ctx.fillRect(50, 50, 420, 480); ctx.strokeStyle = '#e74c3c'; ctx.lineWidth = 4; ctx.strokeRect(50, 50, 420, 480);
        ctx.fillStyle = '#e74c3c'; ctx.font = 'bold 24px Arial'; ctx.textAlign = 'center'; ctx.fillText("MENU DEBUG", 260, 90);
        ctx.textAlign = 'left'; ctx.font = '20px Arial';
        debugOptions.forEach((opt, i) => {
            let text = opt; if (i === 4) text += ` : [${nextType}]`;
            if (i === selectedIndex) { ctx.fillStyle = '#f1c40f'; ctx.fillText(`▶ ${text}`, 70, 140 + (i * 35)); } else { ctx.fillStyle = 'white'; ctx.fillText(`  ${text}`, 70, 140 + (i * 35)); }
        });
        ctx.fillStyle = '#7f8c8d'; ctx.font = 'italic 16px Arial'; ctx.fillText("Z/S: Naviguer | Entrée: Valider | C: Quitter", 70, 500);
    }
};