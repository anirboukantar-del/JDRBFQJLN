...
import { getWeaponBoost, getArmorPpBoost, getGradeMultiplier, calculatePpMult, type EquipmentItem } from './inventory';
...
export let activeEnemies: any[] = []; export let activePlayer: any = null; 
export let currentTargetIndex = 0; let currentEnemyActionIndex = 0; 
export let isIntroAnimating: boolean = false; export let introAnimationTimer: number = 0; export const INTRO_ANIMATION_DURATION = 60;
export let gracePeriodTimer = 0; export const combatMenuOptions = ['Attaque', 'Spé', 'Défendre', 'Fuite']; export let currentMenuIndex = 0; 
...
export let currentSkillIndex = 0; export let pendingSkill: Skill | null = null; 
...
function positionEntitiesForCombat(player: any, enemies: any[]) {
    ...
    const padding = 55; const totalGroupHeight = enemies.reduce((sum, e) => sum + e.height, 0) + ((enemies.length - 1) * padding);
...
function applyTurnEndModifiersWithMessages(entity: any) {
    if (entity.activeModifiers.includes('burning')) { entity.takeDamage(Math.floor(entity.maxHp * 0.05)); entity.addFloatingText(Math.floor(entity.maxHp * 0.05).toString(), '#e74c3c'); }
    if (entity.activeModifiers.includes('poisoned')) { entity.takeDamage(Math.floor(entity.maxHp * 0.08)); entity.addFloatingText(Math.floor(entity.maxHp * 0.08).toString(), '#9b59b6'); }
    
    // Dissipation aléatoire des nouveaux statuts (33% de chance de disparaître par tour)
    const decayChance = 0.33;
    if (entity.activeModifiers.includes('cloned') && Math.random() < decayChance) { entity.activeModifiers.splice(entity.activeModifiers.indexOf('cloned'), 1); entity.addFloatingText("Clone dissipé", "#bdc3c7"); }
...
function checkDeath() {
    ...
    for (let i = activeEnemies.length - 1; i >= 0; i--) {
        if (activeEnemies[i].hp <= 0) {
            ...
            activeEnemies.splice(i, 1); if (currentTargetIndex >= activeEnemies.length) currentTargetIndex = Math.max(0, activeEnemies.length - 1);
        }
    }
    if (activeEnemies.length === 0) { CombatSystem.end(false); return true; } return false;
}
...
function calculateDamage(attacker: any, defender: any, skill: Skill | null, isDefenderGuarding: boolean) {
    ...
    if (skill) {
        ...
        // --- NOUVEAUTÉ : Double Dégâts (Attaque Clone) ---
        if (attacker.activeModifiers.includes('cloned')) { rawDmg *= 2; effectMsg = effectMsg ? effectMsg + " (x2 Clone)" : "x2 Clone!"; }

        rawDmg *= variance(); splits.push({ damage: Math.max(1, Math.floor(rawDmg)), element: skill.element });
    } else {
...
export const CombatSystem = {
    ...
    executeNextStep() {
        ...
        if (executionStep === 0) {
            activePlayer.isDefending = false; activePlayer.isActing = true;

            if (activePlayer.activeModifiers.includes('feared')) {
                activePlayer.activeModifiers.splice(activePlayer.activeModifiers.indexOf('feared'), 1);
            ...
            if (queuedPlayerAction!.type === 'FLEE') { this.end(true); return; } 
                ...
                activePlayer.isDefending = true; activePlayer.currentActionName = { text: "Garde", color: "white", timer: 80 };
                const burnIdx = activePlayer.activeModifiers.indexOf('burning'); if (burnIdx !== -1) activePlayer.activeModifiers.splice(burnIdx, 1); 
                const mudIdx = activePlayer.activeModifiers.indexOf('muddy'); if (mudIdx !== -1) activePlayer.activeModifiers.splice(mudIdx, 1); 
                // Retirer Attendrissement
                ...
                if (skill && skill.id === 'meditation') {
                    ...
                    targets.forEach(t => {
                        ...
                        // On applique toujours les dégâts (Ragebait et Brainrot ont PWR=0, totalDamage sera ~0)
                        ...
                        if (skill && skill.modifiers) skill.modifiers.forEach((modId: string) => { if (!t.activeModifiers.includes(modId)) t.activeModifiers.push(modId); });
                    });
                }
            }
            ...
            if (currentEnemyActionIndex < activeEnemies.length) {
                ...
                currentEnemy.isDefending = false; currentEnemy.isActing = true;

                if (currentEnemy.activeModifiers.includes('feared')) {
                    currentEnemy.activeModifiers.splice(currentEnemy.activeModifiers.indexOf('feared'), 1);
                ...
                // --- NOUVEAUTÉ : Restrictions de l'IA (Brainrot / Ragebait) ---
                ...
                if (action.type === 'DEFEND') {
                    currentEnemy.isDefending = true; currentEnemy.currentActionName = { text: "Garde", color: "white", timer: 100 };
                    ...
                    const enemySkill = action.skill || null; let PWR = enemySkill ? enemySkill.pwr : 1;
                    ...
                    if (enemySkill) { if (enemySkill.targetType === 'all_allies') targets = [...activeEnemies]; else if (enemySkill.targetType === 'self') targets = [currentEnemy]; }
                    ...
                    if (enemySkill && enemySkill.id === 'meditation') {
                        ...
                        targets.forEach(t => {
                            ...
                            if (enemySkill && enemySkill.modifiers) enemySkill.modifiers.forEach((modId: string) => { if (!t.activeModifiers.includes(modId)) t.activeModifiers.push(modId); });
            ...
            if (checkDeath()) return; combatSubState = 'ACTION_SELECT'; currentMenuIndex = 0; 
        }
    },
    ...
    handleInput(key: string) {
        ...
        if (combatSubState === 'ACTION_SELECT') {
            ...
            if (key === 'ArrowUp' || key === 'z' || key === 'Z') { currentSkillIndex--; if (currentSkillIndex < 0) currentSkillIndex = Math.max(0, skills.length - 1); } 
                ...
                if (skills.length > 0) {
                    const sObj = skills[currentSkillIndex]; const dSkill = getDynamicSkill(sObj.id, sObj.level);
                    const actualCost = Math.floor(dSkill.ppCost * calculatePpMult(activePlayer));

                    if (activePlayer.pp >= actualCost) {
                        pendingSkill = dSkill; 
                        if (dSkill.targetType !== 'single') { queuedPlayerAction = { type: 'SKILL', skill: pendingSkill }; this.startExecutionPhase(); } 
                        else { combatSubState = 'TARGET_SELECT'; currentTargetIndex = 0; }
                    }
                }
            ...
            else if (key === 'Enter' || key === ' ') { queuedPlayerAction = { type: pendingSkill ? 'SKILL' : 'ATTACK', skill: pendingSkill }; this.startExecutionPhase(); } 
            else if (key === 'Escape' || key === 'Backspace') { if (pendingSkill) combatSubState = 'SKILL_SELECT'; else combatSubState = 'ACTION_SELECT'; pendingSkill = null; }
        }
    },
