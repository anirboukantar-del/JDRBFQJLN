...
import { getWeaponBoost, getArmorPpBoost, type EquipmentItem } from './inventory';
...
function calculateDamage(attacker: any, defender: any, skill: Skill | null, isDefenderGuarding: boolean) {
    ...
    // Un peu plus de variance pour le piment (85% à 115%)
    const variance = () => 0.85 + (Math.random() * 0.3); 
    const hasUltraGarde = defender.activeModifiers.includes('ultra_garde');

    // --- NOUVEAUTÉ : Le Facteur d'Échelle ---
    // Cette variable permet d'avoir de vrais chiffres de JRPG au lieu de "1" ou "2"
    const baseDamageScale = 4 + (attacker.level * 0.5); 

    if (skill) {
        let coefficient = 1.0; if (weapon && weapon.spAtkBonus) coefficient *= (1 + weapon.spAtkBonus);
        ...
        // On multiplie par le facteur d'échelle à la fin
...
function applyElementalEffects(attacker: any, defender: any, damage: number, skill: Skill | null) {
    ...
    if (element === 'normal' && !skill) return "";
    ...
    if (skill && skill.id === 'cri' && Math.random() < 0.35 + (skill.level * 0.02)) { if (!defender.activeModifiers.includes('feared')) defender.activeModifiers.push('feared'); return " 😨 Terrorisé !"; }
    if (skill && skill.id === 'frappe_terreur' && Math.random() < 0.50 + (skill.level * 0.02)) { if (!defender.activeModifiers.includes('feared')) defender.activeModifiers.push('feared'); return " 😨 Terrorisé !"; }
    ...
    executeNextStep() {
        ...
        if (executionStep === 0) {
            ...
            if (queuedPlayerAction!.type === 'FLEE') { this.end(true); return; } 
                ...
                const ppMult = 1 + (weapon?.spCostPenalty || 0);
                if (skill) activePlayer.pp -= Math.floor(skill.ppCost * ppMult);
                ...
                if (PWR < 0) { 
                    ...
                    targets.forEach(t => {
                        ...
                        applyElementalEffects(activePlayer, t, result.totalDamage, skill);
                    });
                }
            }
            ...
            if (currentEnemyActionIndex < activeEnemies.length) {
                ...
                const action = EnemyAI.decideAction(currentEnemy, ppMult);
                ...
                if (action.type === 'DEFEND') {
                    ...
                    if (enemySkill) currentEnemy.pp -= Math.floor(enemySkill.ppCost * ppMult);
                    ...
                    if (PWR < 0) { 
                        ...
                        targets.forEach(t => {
                            ...
                            t.takeDamage(result.totalDamage); result.splits.forEach((split: any) => t.addFloatingText(split.damage.toString(), elementColors[split.element]));
                            if (result.effectMsg) t.addFloatingText(result.effectMsg, '#f1c40f', true);
                            applyElementalEffects(currentEnemy, t, result.totalDamage, enemySkill);
